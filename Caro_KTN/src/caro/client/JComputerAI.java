...
import static caro.client.JComputer.WHITE;
import caro.common.CPiece;
import caro.common.GPos;

public class JComputerAI {
    ...
    public JComputerAI(JGobanAI goban) {
    ...
    public int Think(int color, GPos Best) {
        ...
        for (x = 0; x < JGoban.BOARDSIZE; x++) {
            for (y = 0; y < JGoban.BOARDSIZE; y++) {
        ...
        for (x = 0; x < JGoban.BOARDSIZE; x++) {
            for (y = 0; y < JGoban.BOARDSIZE; y++) {
                if (Goban.Pieces[x][y].State == CPiece.EMPTY && Goban.Area[x][y] > 0) {
                    ...
                    for (x2 = 0; x2 < JGoban.BOARDSIZE; x2++) {
                        for (y2 = 0; y2 < JGoban.BOARDSIZE; y2++) {
    ...
    public int GetSequence(int color, int x, int y, int direction) {
        ...
        while (Goban.Pieces[x][y].State == color) {
            ...
            x += dx;
            y += dy;
            if (x < 0 || x >= Goban.BOARDSIZE || y < 0 || y >= Goban.BOARDSIZE) {
                break;
            }
    ...
    public int GetGlobalScore(int color) {
        ...
        for (y = 0; y < JGoban.BOARDSIZE; y++) {
            ...
            } while (x < JGoban.BOARDSIZE);
        }

        for (int i = 0; i < JGoban.BOARDSIZE * 2 - 1; i++) {
            int ret;
            if (i < JGoban.BOARDSIZE) {
                x = 0;
                y = i;
            } else {
                x = i - JGoban.BOARDSIZE + 1;
                y = 0;
            }
            spacecheck = false;
            do {
            ...
            } while (x < JGoban.BOARDSIZE && y < JGoban.BOARDSIZE);
        }
